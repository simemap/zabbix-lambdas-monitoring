...
ZabbixSender.prototype.send = function(callback) {
    ...
    client.on('timeout', function() {
        error = new Error("socket timed out after " + self.timeout / 1000 + " seconds, started at:" + startTime.toISOString());
        client.destroy();
    });
    ...
    client.on('close', function() {
        ...
        if (response.slice(0, 5).toString() !== 'ZBXD\x01') {
            ...
            self.items = self.items.concat(items);
            return callback(new Error("got invalid response from server: " + host +
                ", result:[" + response + '], started at:' + startTime.toISOString()), {});
        }
...
function prepareData(items, with_timestamps, with_ns) {
    ...
    var payload = new Buffer.from(JSON.stringify(data), 'utf8'),
        header  = new Buffer.alloc(5 + 4); // ZBXD\1 + packed payload.length

    header.write('ZBXD\x01');
    header.writeInt32LE(payload.length, 5);
    return Buffer.concat([header, new Buffer.from('\x00\x00\x00\x00'), payload]);
}